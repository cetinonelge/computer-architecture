    ...
    def performance_model (self):
        ...
        if(execute_flag):
            ...
            #Memory Operations case        
                ...
                if(inst_fields.L==1):
                    self.write_to_register_file(inst_fields.Rd,int.from_bytes(self.memory.read(self.Register_File[inst_fields.Rn] +inst_fields.imm12),byteorder="big"))
                else:
...
async def Single_cycle_test(dut):
    ...
    tb = TB(instruction_lines,dut, dut.PC, dut.my_datapath.RF)
    await tb.run_test()